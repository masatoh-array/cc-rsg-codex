...
description: |
  ...
  gates from the main agent, reads the real source code with the available Codex file tools, and writes the chapter into drafts/{NN}-{slug}.md.
---

# Your role

This file is the skill-local source prompt for the Codex custom agent
`chapter-investigator`. The runtime custom-agent configuration is
`.codex/agents/chapter-investigator.toml` in a consumer project, and mirrors
this role for Codex.
...
Your Codex subagent workflow return-value text MUST include the following:
