...
# cc-rsg (Codex Reverse Spec Generator)
...
> (chapter body, user-facing prompt bodies in Codex, progress messages, etc.) in
...
8. **The dialogue protocol is agent-driven**: Choice-based questions are the default; free-form input is a fallback. Codex choice-oriented prompts are used whenever practical.
...
11. **Output language is chosen in Phase 0 (English / 日本語) — English is the BASE language**: The very first dialogue is bilingual (English first, then Japanese). The default selection is provided by the parent harness's initial prompt (the user's UI-language hint; falls back to `"en"`). The answer is persisted to `.cc-rsg/goal.json` as `output_language` (`"en"` or `"ja"`). All subsequent natural-language output — user-facing prompt bodies and choices in Codex, progress messages, confirmation summaries, generated spec body and chapter titles, `questions.json` `body` / `answer`, Phase 4 verification reports, resume messages — uses that language. Internal identifiers and machine-readable elements — state keys (`current_phase` etc.), IDs (`Q-XXX` / `INV-XXX`), file names (ASCII slug), `[REF: file:lines]`, `[CONFIDENCE: HIGH|MED|LOW]`, `[ASK SME]`, `[ASSUMED: ...]`, `[BLOCKED: ...]` marker names, and `goal.json` enum values (`primary_reader: "maintenance_developer"` etc.) — stay English **regardless of `output_language`**. The literal `## Sources Read` heading also stays English (so `coverage-check.py` pattern-matches it). The entire skill bundle (this SKILL.md, `agents/`, `variants/`, `templates/*.md`, `references/*.md`, and `scripts/*.py` docstrings / messages) is English-base. When `output_language == "ja"`, the agent dynamically renders deliverable text (chapter body, user-facing prompt bodies in Codex, progress messages, etc.) in Japanese while preserving every machine-readable element verbatim.
...
3. **Output language selection**
   ...
   - Ask the user in the current Codex conversation with:
     ...
     - Choices (**fixed order; English is the default**):
       1. `English`
       2. `日本語 (Japanese)`
     - Single selection only; no free-form input
   ...
   - **All natural-language output from Step 4 onward** — user-facing prompt bodies and choices, confirmation summaries, chapter titles, generated spec body, `questions.json` body text, etc. — is rendered in the language selected here (see Design Principle #11).
...
4. **Run the 5 goal-definition questions**
   - Ask the following 5 questions in sequence in the current Codex conversation. **Question bodies, choice labels, and free-form-input placeholders are all rendered in the `output_language` selected in Step 3.** The choice labels below are shown when `output_language == "en"`; the agent dynamically translates them when `output_language == "ja"` (enum values such as `primary_reader: "maintenance_developer"` stay as language-independent English enums in `goal.json`). Each question is choice-based first with a free-form field as a fallback.
   - **Question-text quality contract (applies to every user-facing prompt in every phase, especially when translating into `output_language == "ja"`)**:
     ...
     5. These rules apply to **user-facing prompt bodies and choices**, but they do NOT relax the rule that JSON keys, enum values, file names, and machine-readable markers stay English (see Principle #11).
...
2. **Present template candidates**
   ...
   - Present the candidates to the user in the current Codex conversation.
...
5. **🆕 depth-mode decision (scale-based)**
   ...
   - **If file count > 200**, ask the user in the current Codex conversation to choose a **depth mode**:
...
1. **Apply the chapter file naming convention and generate the skeleton**
   ...
   **Forbidden in Phase 2 skeletons** (writing any of these is a contract violation and rolls back the file):
   ...
   - ❌ Entity / module / route / endpoint tables (those are Phase 3 STEP A-C outputs based on real `file-listing`/`search`/`view` reads of the codebase).
   ...
   **Example of a violating skeleton** (do NOT do this in Phase 2 — EN shown for illustration):
   ...
   Even though the table looks plausible, it was written without reading `issue.rb` — the 🟡 label means "search hit only, body unread", which Phase 2 has no business claiming. Save this for Phase 3 where the table cells get grounded in actual `view` output.
...
For every INV in that chapter's `wbs.json.chapters[*].assigned_inventory_ids`, **read the corresponding real source files with the available Codex file tools**.
...
#### STEP G: Per-chapter custom-agent delegation (use when Codex subagents are available; recommended)

In environments where Codex subagents are available, **delegate each chapter to the `chapter-investigator` custom agent**. Its Codex custom-agent configuration must be available as `.codex/agents/chapter-investigator.toml` in the consumer project or as a user-level custom agent.

Writing every chapter directly in the main agent degrades context; investigating each chapter in its own isolated subagent context yields higher quality.

**Message to provide when asking Codex to spawn the `chapter-investigator` custom agent:**

```text
Spawn the `chapter-investigator` custom agent for this chapter.

Chapter: Chapter 5: Data Model
...
When done, return the chapter's key points and detail questions.
```

**Important constraints**:

- **Launch one `chapter-investigator` subagent per chapter in the same assistant turn whenever the runtime supports parallel dispatch.**
  Sequentially launching and waiting for one chapter at a time serialises Phase 3 and wastes the subagent concurrency pool.
- **Self-check before launching subagents**: have you prepared the prompt for every chapter that needs investigation in this Phase 3 round? If not, finish all prompts first, then launch the batch.
- **Prompt cache is NOT shared**: each subagent has an isolated LLM context, so token usage is 5-10x the main agent.
- **The subagent writes the chapter draft directly with the available Codex file-editing tools** (saved as a file, NOT returned in the subagent result text). The main agent reads the return value and appends detail questions into `questions.json`.
- **One subagent per chapter**. Bundling all chapters into one subagent request defeats the purpose because the isolated context per chapter disappears.

**When Codex subagents or the `chapter-investigator` custom agent are unavailable**, the main agent performs STEP A-F itself per chapter.
...
2. **Use `file-listing` + `search` to mechanically extract every entity**:
   - Ruby/Rails models: `search "^class \\w+" --type ruby app/models/`
   - Controllers: `search "^class \\w+Controller" --type ruby app/controllers/`
...
4. Always add a **Confidence label** in each cell (🟢 VERIFIED / 🟡 INFERRED / 🔴 ASSUMED):
   - 🟢: the file of that entity was confirmed by reading it with the available Codex file tools
   - 🟡: only the `search` hit was confirmed; body unread
...
Present **a single question** to the user in the current Codex conversation:
...
Use choices: `["Answer all", "Critical only", "Skip with abandoned"]`, with free-form input allowed.
...
Present each cluster as one user-facing prompt in the current Codex conversation:
```
...
Present each question in the current Codex conversation:
...
If the answer makes additional investigation necessary, re-read the relevant code with the available Codex file tools as an extra step in Phase 3 and update the chapter.
...
- Zero user-facing Phase 5 prompts have been sent in the Codex conversation, OR
...
2. Verify at least the **Stage 1 overview** AND the **Stage 2 critical clusters** dialogues were actually presented to the user in the Codex conversation (Stage 3 individual questions for the residual). Internal notes or `state.json.phase_5.user_feedback` strings do NOT substitute for actual dialogue.
...
6. **Intent-vs-delivery audit (mandatory; the final gate before completion)**
   ...
   - **Verify state.json invariants**:
     ...
     - The Phase 5 skip-prevention conditions (see Phase 5 "skip prevention" section) must hold: `questions.json` open-ratio ≤ 20%, ≥ 1 user-facing Phase 5 prompt sent in the Codex conversation, ≥ 1 question with populated `answer` field.
   ...
   - If after additional Phase 3/4 iterations the agent still cannot deliver a `user_custom` chapter (e.g. the source code does not support it), ask the user in the current Codex conversation for explicit permission to drop the deliverable; only an explicit user opt-out justifies skipping the file. Record the decision in `state.json.phase_6.user_opt_outs[]` with the reason.
...
2. **Direct entity name**: `Issue class` / `ProjectsController`, etc. → identify the file via `search`.
...
1. Launch the `chapter-investigator` custom agent via the Codex subagent workflow.
2. Sub-agent prompt:
...
The prompt skeleton handed to the Codex subagent workflow in Phase 3. The complete version lives in `references/subagent-prompt.md`.
...
├── SKILL.md                          (this file)
├── agents/
│   └── chapter-investigator.md        (source prompt for the Codex custom agent)
├── references/
│   ├── inventory-units.md
│   ├── template-catalog.md
...
The Codex custom-agent runtime definitions are distributed alongside the skill:

```
skills-for-codex/.codex/agents/
├── chapter-investigator.toml
└── chapter-investigator-mode-b.toml
```
...
- v0.5.0 (2026-06-15): intent-vs-delivery enforcement + post-pilot quality hardening.
  ...
  - **Phase 5 skip prevention**: refusing `complete` when `status: open` ratio ≥ 20%, no user-facing Phase 5 prompt was sent in the Codex conversation, or zero `answer` fields are populated.
  ...
  - **Question-text quality contract**: no `\uXXXX` JSON escapes in user-facing prompt text, kanji confusion-pair self-check (`妥/妊`, `暑/署`, `復/複`, ...).
  - **Optional `variants/B/`** for Context Optimization mode B (per-chapter custom-agent delegation with manifest relay) — opt-in via `goal.json.context_optimization_mode = "B"`.
