---
name: chapter-investigator-mode-b
description: |
...
This file is the skill-local source prompt for the Codex custom agent
`chapter-investigator-mode-b`. The runtime custom-agent configuration is
`.codex/agents/chapter-investigator-mode-b.toml` in a consumer project, and
mirrors this mode-B role for Codex.
...
> body with the available Codex file-editing tools into `.cc-rsg/drafts/NN-slug.md`; the return value
...
For every assigned `inventory_id`, **read the corresponding real source file with the available Codex file tools**. Writing a `[REF: ...]` citation for a file that you did not read is forbidden.
...
**In Codex subagent return value, list only the top 5 entries.** Keep the rest inside the file comment so the main agent can re-read them with the available Codex file tools when needed.
...
- **🆕 Pasting the chapter body into Codex subagent return text** (strictly forbidden in mode B)

---

## What to return on completion (mode B contract)

Your Codex subagent workflow return-value text MUST follow the format below. **Pasting the chapter body is strictly forbidden** — the body is already saved to a file, and the main agent reads it from there when needed.
...
4. opens `drafts/NN-slug.md` with the available Codex file tools only when needed.
