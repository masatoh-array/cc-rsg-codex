...
Full template for the prompt handed to the `chapter-investigator` custom agent launched in Phase 3 via the Codex subagent workflow.
...
[5. Task instructions]
================================
1. Use the available Codex file tools to carefully read the source files corresponding to the assigned inventory items.
2. As needed, use search and file-listing tools to explore related code.
...
[6. Output format]
================================
Write `drafts/{output_file_name}` as Markdown shaped as follows. The frontmatter (including `output_file_name`) is mandatory in the file.
...
- Before fully reading a file, narrow it down with search first.
  For files under 100 lines, a full Read is fine.
- Use official-doc lookup or web search only to consult the official docs of an external library.
...
1. The generated chapter draft path and quality counts.
2. The detail-questions list (structured summary).
3. If any sections are blocked, the list of them.
4. Any unexpected situations you encountered.

Do not paste the generated chapter body into the return text. The main agent reads the draft file when it needs the body.
```
    ...
    for section in chapter_sections:
        try:
            ...
            if q.severity == "critical":
                blocked_sections.append(section.name)
                content = f"[BLOCKED: see {q.id}]"
            else:
                content = generate_with_assumption(section, q)
                # Marked with [CONFIDENCE: LOW; ASSUMED: ...]

    # 3. Append the question list at the end of the chapter and write the draft
    draft = chapter_draft + format_questions(questions)
    write_draft(f"drafts/{output_file_name}", draft)

    # 4. Return only the completion report
    return completion_report(
        path=f"drafts/{output_file_name}",
        questions=questions,
...
- First narrow with search, then Read only the relevant line ranges.
...
def launch_subagents(wbs, goal, inventory):
    ...
    for chapter in wbs.chapters:
        ...
        task = spawn_subagent(
            agent_type="chapter-investigator",
    ...
    for result, chapter in zip(results, wbs.chapters):
        assert_draft_exists(f"drafts/{chapter.file_name}")
...
- [ ] No detail mentions of out-of-scope inventory items (cross-check with search).
